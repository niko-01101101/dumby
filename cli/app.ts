...
import { Manager, setViewOnly } from '#core/manager';
import { ContentCreator } from '#core/contentCreator';
import { Editor } from '#core/editor';
import { Account } from '#core/account';
import { Video } from '#core/video';
import { refreshListItems, setBlockLifecycleActions } from './displays/entityDisplay.ts';
...
// How often the dashboard re-pulls state from the DB (Manager.refreshTree(),
// see manager.ts) — a headless daemon process (cli/daemon.ts) is the one
// actually mutating these rows, and Entity.on("change", ...) is in-process
// only, so a second process has no other way to notice.
const DASHBOARD_POLL_INTERVAL_MS = 3_000;

export let screen: Widgets.Screen;
let currentBack: () => void = () => { };
...
function buildRows(manager: Manager | undefined): Row[] {
  if (!manager) return [];
  const out: Row[] = [{ entity: manager, depth: 0 }];
  for (const cc of manager.contentCreators) {
    if (!cc) continue;
    out.push({ entity: cc, depth: 1 });
    for (const account of cc.accounts) {
      if (!account) continue;
      out.push({ entity: account, depth: 2 });
      for (const video of account.videos) {
        if (!video) continue;
        out.push({ entity: video, depth: 3 });
      }
    }
  }
  for (const editor of manager.editors) {
    if (!editor) continue;
    out.push({ entity: editor, depth: 1 });
  }
  return out;
}
...
export interface StartAppOptions {
  // A dashboard process: skips Manager's autostart cascade and the Reminder
  // scheduler (a separate headless daemon, cli/daemon.ts, owns both), drops
  // just the Start/Shutdown action from every display (plain data actions —
  // Add/Edit/Archive/Delete/Connect — stay enabled, see entityDisplay.ts's
  // blockLifecycleActions), and polls the DB on an interval (Manager.
  // refreshTree()) to reflect that daemon's state instead.
  viewOnly: boolean;
}

export async function startApp(opts: StartAppOptions): Promise<void> {
  if (opts.viewOnly) {
    setViewOnly(true);
    setBlockLifecycleActions(true);
  }

  const manager = await Manager.load("main");
  // onLoad() already loads contentCreators/editors and, unless viewOnly,
  // starts the Manager — see manager.ts.

  screen = blessed.screen({
    smartCSR: true,
    title: opts.viewOnly ? 'dumby (read-only)' : 'dumby',
  });

  const summaryBox = blessed.box({
    parent: screen,
    label: opts.viewOnly ? ' dashboard (read-only) ' : ' dashboard ',
    top: 0,
    width: '100%',
    height: 3,
    tags: true,
  ...
  const stopScheduler = opts.viewOnly ? undefined : startScheduler();

  let pollTimer: ReturnType<typeof setInterval> | undefined;
  if (opts.viewOnly && manager) {
    pollTimer = setInterval(() => {
      manager.refreshTree()
        .then(sync)
        .catch((e: unknown) => console.error(`Dashboard refresh failed: ${e instanceof Error ? e.message : String(e)}`));
  ...
  async function quit() {
    ...
    stopScheduler?.();
    // A read-only dashboard never started anything, so it shouldn't shut
    // anything down either — that's the daemon process's job.
    if (!opts.viewOnly) await manager?.shutdown();
